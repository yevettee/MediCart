...
// 필드 키 = RTDB visit 레코드 키 (저장 시 환자정보에 그대로 반영).
type Field =
...
export const INTAKE_SECTIONS: { n: string; title: string; fields: Field[] }[] = [
...
/** 외래 방문 문진 폼(의료진 addVisit 경로) — /intake 와 동일 필드, 임베드용. */
export default function IntakeForm({ pid, patientName, onSaved, onCancel }: Props) {
  const [form, setForm] = useState<Record<string, unknown>>({ 방문일: today() });
  const [busy, setBusy] = useState(false);
  const [err, setErr] = useState("");
  // pid 별 초기화는 호출부에서 key={pid} 로 리마운트해 처리(effect 내 setState 회피).
  ...
  return (
    <div className="w-full max-w-[760px] mx-auto bg-surface text-ink rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[88vh]">
      ...
      <div className="px-6 py-3.5 border-t border-line flex items-center justify-end gap-3 shrink-0">
        {err && <span className="pill bg-red-soft text-red"><span className="dot bg-red" /> {err}</span>}
        {onCancel && (
...
export function FieldInput({ f, value, set }: { f: Field; value: unknown; set: (id: string, v: unknown) => void }) {
  ...
  if (f.type === "textarea")
    return <textarea className="field min-h-[70px] resize-y" value={(value as string) ?? ""} onChange={(e) => set(f.id, e.target.value)} />;
  if (f.type === "select")
    return (
